...
Topic :         Multi Byte Data with I2C1
Filename:       I2C Master Multi Byte Write Rev 0.0
...
int main()
{
  ...
  writeByte(0x00, (1<<0)|(1<<1));                       // writeByte(DATA ,(ENABLE|START|STOP), Write the DATA onto the bus. BYTE1
  writeByte(0x00, (1<<0));                              // writeByte(DATA ,(ENABLE), Write the DATA onto the bus. BYTE2
  writeByte(0x00, (1<<0));                              // writeByte(DATA ,(ENABLE), Write the DATA onto the bus. BYTE3
  writeByte(0x00, (1<<0)|(1<<2));                       // writeByte(DATA ,(ENABLE), Write the DATA onto the bus. BYTE4
  
  while(1)
  {
    writeByte(0x10, (1<<0)|(1<<1));                     // writeByte(DATA ,(ENABLE|START|STOP), Write the DATA onto the bus. BYTE1
    writeByte(0x11, (1<<0));                            // writeByte(DATA ,(ENABLE), Write the DATA onto the bus. BYTE2
    writeByte(0x00, (1<<0));                            // writeByte(DATA ,(ENABLE), Write the DATA onto the bus. BYTE3
    writeByte(0x8F, (1<<0)|(1<<2));                     // writeByte(DATA ,(ENABLE), Write the DATA onto the bus. BYTE4
    for(int i = 0; i < 1000; i++);                      // Arbitrarily a delay is given. Not need for this if timed delay desired
  }
  return 0;
}
